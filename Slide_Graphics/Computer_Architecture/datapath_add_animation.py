...
Generates an animated GIF that walks students through how an ADD (R-type)
...
Stages animated:
    ...
    3. The instruction is broken apart:
         - opcode bits [31-21]      -> Control unit  (shown as "ADD")
         - Rn [9-5] / Rm [20-16]    -> register-file read ports
         - Rd [4-0]                 -> register-file write port
    4. The register file reads out the two source operands (made-up data).
    5. Operands travel through the ALUSrc mux into the ALU.
    6. The ALU adds them and produces a result.
    7. The result loops back (through the WB mux) into the destination register.

The full instruction and its field breakdown stay on screen the whole time.

Run:
    python3 datapath_add_animation.py

Output:
    datapath_add_animation.gif
"""
...
# --------------------------------------------------------------------------- #
#  Example instruction + made-up register contents
# --------------------------------------------------------------------------- #
OPCODE = "ADD"
INSTR_TEXT = "ADD X1, X2, X3"
RN_VAL = 25          # contents of X2 (source 1)
RM_VAL = 17          # contents of X3 (source 2)
RESULT = RN_VAL + RM_VAL  # 42 -> written back to X1
...
CTRL_DEFS = [
    ("Reg2Loc",  [(7.60, 8.10), (6.475, 8.10), (6.475, 5.80)], (6.30, 7.55, "right"), {3, 5}),
    ("RegWrite", [(8.35, 7.95), (8.35, 6.50)],                  (8.18, 7.20, "right"), {8}),
    ("ALUSrc",   [(9.50, 8.30), (10.70, 8.30), (10.70, 4.95)],  (10.82, 6.85, "left"), {5, 6}),
    ("ALUOp",    [(9.55, 8.50), (12.55, 8.50), (12.55, 6.30)],  (12.40, 7.30, "right"), {6}),
    ("MemRead",  [(9.60, 8.68), (14.60, 8.68), (14.60, 5.80)],  (14.70, 7.45, "left"), set()),
    ("MemWrite", [(9.62, 8.84), (16.20, 8.84), (16.20, 5.80)],  (16.05, 8.05, "right"), set()),
    ("MemToReg", [(9.64, 9.00), (17.10, 9.00), (17.10, 5.70)],  (16.98, 6.60, "right"), {7, 8}),
...
ref_lines = [
    ...
    f"Rn  [9-5]   = X2   -> reads {RN_VAL}",
    f"Rm  [20-16] = X3   -> reads {RM_VAL}",
    f"Rd  [4-0]   = X1   <- writes {RESULT}",
    f"Operation:  X1 = X2 + X3  =  {RESULT}",
]
...
STAGES = [
    ...
    ("3a. Decode - opcode -> Control",
     "Bits [31-21] decode to opcode ADD and set the datapath control signals.",
     [(split_x, 5.0), (split_x, 8.2), ctrl_in], OPCODE, CTRL, None, None),

    ("3b. Decode - source registers",
     "Rn = X2 -> Read reg 1  and  Rm = X3 -> Read reg 2 index the register file's two read ports.",
     [(split_x, 5.0), (split_x, 6.10), rr1_in], "Rn=X2", REGS,
     [(split_x, 5.0), (split_x, 5.10), (REGMUX[0], 5.10),
      (REGMUX[0] + REGMUX[2], 5.10), rr2_in], "Rm=X3"),

    ("3c. Decode - destination register",
     "Rd = X1 is routed to the register file's Write-register port.",
     [(split_x, 5.0), (split_x, 4.20), wr_in], "Rd=X1", REGS, None, None),

    ("4. Register Read",
     f"Read data 1 = X2 = {RN_VAL} and Read data 2 = X3 = {RM_VAL} leave the register file together and head to the ALU.",
     [rd2_out, (ALUMUX[0], 4.40), alumux_out, alu_in_bot],
     f"X3 = {RM_VAL}", REGS,
     [rd1_out, (11.4, 5.90), alu_in_top], f"X2 = {RN_VAL}"),

    ("5. Execute (ALU)",
     f"Inside the ALU both inputs are added:  Read data 1 = {RN_VAL}  +  Read data 2 = {RM_VAL}.",
     [alu_in_bot, (12.4, 4.5)], f"{RM_VAL}", ALU,
     [alu_in_top, (12.4, 5.4)], f"{RN_VAL}"),

    ("6. ALU Result",
     f"The ALU produces the result {RESULT} and sends it to the write-back mux.",
     [alu_out, (13.6, 5.00), (13.6, 3.4), (16.7, 3.4), (16.7, 4.65), (WBMUX[0], 4.65)],
     f"= {RESULT}", ALU, None, None),

    ("7. Write Back",
     f"RegWrite is asserted; {RESULT} is written back into destination register X1.",
...
out = "datapath_add_animation.gif"
