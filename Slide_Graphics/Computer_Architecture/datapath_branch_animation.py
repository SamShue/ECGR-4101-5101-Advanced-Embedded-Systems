...
Generates an animated GIF that walks students through how a CBZ (branch)
instruction flows through a simple single-cycle datapath:

    CBZ X1, #16      ; if X1 == 0 then PC <- PC + 4 + (16 << 2)

Stages animated:
    1. PC drives the Instruction Memory (fetch).
    2. The 32-bit instruction is read out.
    3. The instruction is broken apart:
         - opcode bits [31-21]      -> Control unit  (shown as "CBZ")
         - Rt [4-0]                 -> register-file read port / compare source
         - immediate                -> sign-extend and shift-left-2 path
    4. Register value is tested against zero in the ALU.
    5. Branch target address is formed from PC+4 and shifted immediate.
    6. Branch AND Zero drives PCSrc, and the next PC path is selected.

The full instruction and its field breakdown stay on screen the whole time.

Run:
    python3 datapath_branch_animation.py

Output:
    datapath_branch_animation.gif
"""
...
# --------------------------------------------------------------------------- #
#  Example instruction + made-up branch state
# --------------------------------------------------------------------------- #
OPCODE = "CBZ"
INSTR_TEXT = "CBZ X1, #16"
RN_VAL = 0           # value read from X1 (Rt); zero means branch is taken
RM_VAL = 16          # immediate branch offset field
PC_VAL = 400
PC_PLUS4 = PC_VAL + 4
BR_TARGET = PC_PLUS4 + (RM_VAL << 2)
RESULT = BR_TARGET   # reused label slot for final highlighted value
...
CTRL_DEFS = [
    ("Reg2Loc",  [(7.60, 8.10), (6.475, 8.10), (6.475, 5.80)], (6.30, 7.55, "right"), {3}),
    ("RegWrite", [(8.35, 7.95), (8.35, 6.50)],                 (8.18, 7.20, "right"), set()),
    ("ALUSrc",   [(9.50, 8.30), (10.70, 8.30), (10.70, 4.95)], (10.82, 6.85, "left"), {4}),
    ("ALUOp",    [(9.55, 8.50), (12.55, 8.50), (12.55, 6.30)], (12.40, 7.30, "right"), {4}),
    ("MemRead",  [(9.60, 8.68), (14.60, 8.68), (14.60, 5.80)], (14.70, 7.45, "left"), set()),
    ("MemWrite", [(9.62, 8.84), (16.20, 8.84), (16.20, 5.80)],  (16.05, 8.05, "right"), set()),
    ("MemToReg", [(9.64, 9.00), (17.10, 9.00), (17.10, 5.70)], (16.98, 6.60, "right"), set()),
    ("Branch",   [(9.66, 9.12), (9.66, 10.95), (13.75, 10.95), (13.75, 9.67), (14.00, 9.67)], (13.55, 10.62, "right"), {5, 6}),
]
...
ref_lines = [
    ...
    f"Rt  [4-0]   = X1   -> value {RN_VAL}",
    f"imm offset  = #{RM_VAL} (branch immediate)",
    f"PC          = {PC_VAL},  PC+4 = {PC_PLUS4}",
    f"target      = PC+4 + ({RM_VAL} << 2) = {BR_TARGET}",
    f"Branch:     if X1==0 -> next PC = {BR_TARGET}",
]
...
STAGES = [
    ...
    ("3a. Decode - opcode -> Control",
        "Bits [31-21] decode to opcode CBZ and set branch-oriented control signals.",
     [(split_x, 5.0), (split_x, 8.2), ctrl_in], OPCODE, CTRL, None, None),

    ("3b. Decode - Rt + immediate",
     "Rt = X1 is routed through Reg2Loc to Read reg 2, and the branch immediate goes to Sign-extend.",
     [(split_x, 5.0), (split_x, 5.10), (REGMUX[0], 5.10),
      (REGMUX[0] + REGMUX[2], 5.10), rr2_in], "Rt=X1", REGS,
     [(split_x, 5.0), (split_x, 1.9), (SEXT[0], 1.9)], "imm=#16"),

    ("4. Compare to Zero",
     f"The register file outputs Rt = X1 = {RN_VAL}; ALU compares against zero and raises Zero=1.",
     [rd2_out, (ALUMUX[0], 4.40), alumux_out, alu_in_bot],
     f"X1 = {RN_VAL}", ALU,
     [(SEXT[0] + SEXT[2], 1.9), (10.3, 1.9), (10.3, 3.70), (ALUMUX[0], 3.70), alumux_out, alu_in_bot], "0"),

    ("5. Build Branch Target",
     f"Sign-extended immediate is shifted left by 2 and added to PC+4, producing target {BR_TARGET}.",
     [(SL2[0] + SL2[2], 9.84), (12.05, 9.84), (12.05, 9.71), (BADD[0], 9.71), (BADD[0] + BADD[2], 10.20), (14.90, 10.20), (14.90, 10.15), (PCMUX[0], 10.15)],
     f"target {BR_TARGET}", BADD,
     [(3.60, 11.10), (12.15, 11.10), (12.15, 10.59), (BADD[0], 10.59)], f"PC+4={PC_PLUS4}"),

    ("6. Select Next PC",
     "Branch AND Zero asserts PCSrc, so the PC mux selects the branch-target address.",
     [(ANDG[0] + ANDG[2], 9.55), (15.61, 9.55), (15.61, 9.90), (PCMUX[0] + PCMUX[2], 10.625), (16.30, 10.625), (16.30, 11.70), (0.85, 11.70), (0.85, 5.70)],
     "PCSrc=1", PCMUX, None, None),

    ("7. Branch Taken",
     f"Because X1 is zero, next PC becomes the branch target {BR_TARGET}.",
     [(PCMUX[0] + PCMUX[2], 10.625), (16.30, 10.625), (16.30, 11.70), (0.85, 11.70), (0.85, 5.70), pc_out],
     f"PC <- {BR_TARGET}", PC, None, None),
]
...
out = "datapath_branch_animation.gif"
